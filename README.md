...
Led the project’s early technical development by building the 3-RRS inverse kinematics model and simulation workflow in MATLAB, and by exporting the resulting actuator control commands to STM32 for feasibility validation on embedded hardware. He later developed a complete Raspberry Pi-based Python tracking system, including YOLO-based visual target detection, PID control, an inverse kinematics algorithm aligned with the MATLAB prototype, and PCA9685-based servo actuation, which laid the groundwork for the later transition to the C++ version. In addition, he designed the SolidWorks mechanical model, carried out the 3D printing of key structural parts, handled component and motor selection and procurement, developed an MPU6050-based control scheme for servo input testing, and contributed to project presentation materials by editing the team’s first TikTok video.
