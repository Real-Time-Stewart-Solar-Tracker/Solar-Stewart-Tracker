...
#if SOLAR_HAVE_IMU
    /**
     * @brief Construct in IMU mode (bypasses camera/vision/controller pipeline).
     *
     * The IMU publisher reads orientation angles and emits PlatformSetpoint
     * directly into the kinematics stage.
     *
     * @param log    Logger reference
     * @param imu    IMU publisher (ownership transferred)
     * @param kinCfg Kinematics configuration
     * @param actCfg ActuatorManager configuration
     * @param drvCfg ServoDriver configuration
     */
...
#if SOLAR_HAVE_IMU
    // IMU callback (IMU mode only)
    void onImuSetpoint_(const PlatformSetpoint& sp);
#endif

    // Shared callback wiring (called from both constructors)
    void initCommonCallbacks_();
...
#if SOLAR_HAVE_IMU
    std::unique_ptr<IIMUPublisher> imu_;
#endif
