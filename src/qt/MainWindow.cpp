...
// ============================================================
// System rebuild (stops current system, recreates with new mode)
// ============================================================
void MainWindow::setupObservers_() {
...
void MainWindow::rebuildSystem_(solar::app::SensorMode mode) {
    // 1. Stop timers
    ...
    {
        std::lock_guard<std::mutex> lk(ovMtx_);
        haveEst_ = haveSp_ = haveCmd_ = false;
    }
    imgReady_.store(false);

    // 5. Create and start new system
    currentMode_ = mode;
    cfg_.sensor_mode = mode;
    sys_ = solar::app::SystemFactory::makeSystem(log_, cfg_);

    if (!sys_ || !sys_->start()) {
        setStatus_("Status: FAULT – failed to start");
        return;
    }

    // 6. Re-register observers
    setupObservers_();

    // 7. Restart converter (only needed for camera mode)
    if (mode == solar::app::SensorMode::CAMERA) {
        startConverter_();
    }

    // 8. Restart timers
    if (camTimer_)  camTimer_->start(33);
    if (plotTimer_) plotTimer_->start(33);

    // 9. Reset manual state
    manualMode_ = false;
    setManualEnabled_(false);
    setCameraControlsEnabled_(mode == solar::app::SensorMode::CAMERA);
}

// Qt parent-child ownership: widgets/timers created with a parent are deleted automatically.
// No manual delete required.
MainWindow::MainWindow(solar::Logger& log, solar::app::AppConfig& cfg, QWidget* parent)
    : QMainWindow(parent), log_(log), cfg_(cfg)
{
    auto* central = new QWidget(this);
    auto* root = new QVBoxLayout(central);

    // =============================
    // Top row: AUTO / MANUAL / IMU / STOP
    // =============================
    auto* row = new QHBoxLayout();
    btnAuto_   = new QPushButton("AUTO",   central);
    btnManual_ = new QPushButton("MANUAL", central);
    btnIMU_    = new QPushButton("IMU",    central);
    btnStop_   = new QPushButton("STOP",   central);
    btnStop_->setMinimumHeight(48);
    btnIMU_->setMinimumHeight(48);
    row->addWidget(btnAuto_);
    row->addWidget(btnManual_);
    row->addWidget(btnIMU_);
    ...
    // =============================
    // Create and start initial system (camera mode)
    // =============================
    sys_ = solar::app::SystemFactory::makeSystem(log_, cfg_);
    if (!sys_ || !sys_->start()) {
    ...
    // AUTO: camera auto-tracking mode
    connect(btnAuto_, &QPushButton::clicked, this, [this]() {
        if (currentMode_ == solar::app::SensorMode::IMU) {
            // Switch from IMU to camera mode
            rebuildSystem_(solar::app::SensorMode::CAMERA);
            setStatus_("Status: AUTO");
            return;
        }
        sys_->exitManual();
    ...
    // MANUAL: camera on, manual pan/tilt
    connect(btnManual_, &QPushButton::clicked, this, [this]() {
        if (currentMode_ == solar::app::SensorMode::IMU) {
            // Switch to camera mode first, then enter manual
            rebuildSystem_(solar::app::SensorMode::CAMERA);
        }
        sys_->enterManual();
        manualMode_ = true;
    ...
    // IMU: physical tilt controls the platform
    connect(btnIMU_, &QPushButton::clicked, this, [this]() {
        if (currentMode_ == solar::app::SensorMode::IMU) return; // already in IMU mode
        rebuildSystem_(solar::app::SensorMode::IMU);
        setStatus_("Status: IMU – tilt sensor controls platform");
    });
    ...
    connect(btnStop_, &QPushButton::clicked, this, [this]() {
        ...
        btnIMU_->setEnabled(false);
        btnStop_->setEnabled(false);
        setManualEnabled_(false);
        setCameraControlsEnabled_(false);
    });
    ...
    connect(btnThrMinus_, &QPushButton::clicked, this, [this]() {
        ...
        if (sys_) sys_->setTrackerThreshold(static_cast<uint8_t>(t));
    });
...
void MainWindow::updateCamera_() {
    if (currentMode_ == solar::app::SensorMode::IMU) {
        // Show a simple "IMU MODE" indicator in the camera pane
        QImage placeholder(640, 120, QImage::Format_RGB888);
        ...
        f.setBold(true);
        p.setFont(f);
        p.drawText(placeholder.rect(), Qt::AlignCenter, "IMU MODE ACTIVE\nTilt the sensor to move the platform");
        camView_->setPixmap(QPixmap::fromImage(placeholder));
        return;
    }
...
void MainWindow::setCameraControlsEnabled_(bool en) {
    // AUTO/MANUAL/IMU buttons are always enabled so the user can switch modes freely.
    // Only the threshold controls are camera-specific and get disabled in IMU mode.
    btnThrMinus_->setEnabled(en);
    btnThrPlus_->setEnabled(en);
}
