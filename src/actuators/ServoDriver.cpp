...
float ServoDriver::norm_to_pulse_us(float norm, const ChannelConfig& c) const noexcept {
    // Clamp input to expected normalized range
    norm = std::clamp(norm, cfg_.min_norm, cfg_.max_norm);

    const float span = (cfg_.max_norm - cfg_.min_norm);
    float t = 0.0f;
    if (span > 1e-6f) {
        t = (norm - cfg_.min_norm) / span; // -> [0..1]
    }
    t = std::clamp(t, 0.0f, 1.0f);
    if (c.invert) t = 1.0f - t;

    // Map to servo pulse range and clamp again for safety
    const float pulse = c.min_pulse_us + t * (c.max_pulse_us - c.min_pulse_us);
    return std::clamp(pulse,
                      std::min(c.min_pulse_us, c.max_pulse_us),
                      std::max(c.min_pulse_us, c.max_pulse_us));
}
...
void ServoDriver::write_pulse_us(uint8_t channel, float pulse_us) noexcept {
    if (pca_) {
        ...
        // deterministic fallback: log only (no IO beyond logging, rate-limited in apply())
        (void)channel;
        (void)pulse_us;
    }
}
...
void ServoDriver::park_all() noexcept {
    // Park to neutral to reduce mechanical shock / improve safety.
    for (const auto& c : cfg_.ch) {
        write_pulse_us(c.channel, c.neutral_us);
    }
}
...
void ServoDriver::apply(const ActuatorCommand& cmd) {
    ...
    // Convert 3 actuator targets (normalized) -> pulse widths (us)
    const float p0 = norm_to_pulse_us(cmd.actuator_targets[0], cfg_.ch[0]);
    const float p1 = norm_to_pulse_us(cmd.actuator_targets[1], cfg_.ch[1]);
    const float p2 = norm_to_pulse_us(cmd.actuator_targets[2], cfg_.ch[2]);
    ...
    if (cfg_.log_every_n > 0 && (n % cfg_.log_every_n) == 0) {
        log_.info("ServoDriver apply (us): [" +
                  std::to_string(p0) + ", " +
                  std::to_string(p1) + ", " +
                  std::to_string(p2) + "] " +
                  (pca_ ? "(hw)" : "(log-only)"));
